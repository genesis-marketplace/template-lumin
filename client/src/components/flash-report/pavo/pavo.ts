...
import {cellStyle, currencyFormatter, mcFormatter, mcLFormatter} from "../../../utils/formatters";
...
@customElement({
    name: "pavo-main",
    template: html<Pavo>`
        <zero-grid-pro @onGridReady="${(x) => x.pavoGridReady()}"
                       ${ref('pavoGrid')}
        >
            <grid-pro-genesis-datasource
                    resource-name="ALL_LUMIN_PAVO"
                    :deferredGridOptions=${(x) => x.pavoGridOptions}
            ></grid-pro-genesis-datasource>
        </zero-grid-pro>
    `
})
export class Pavo extends  FASTElement{
    ...
    pavoGridOptions = {
     ...
     pavoGridReady() {
        console.log("lumin|pavo|pavoGridReady")
         this.pavoGrid.gridApi.setColumnDefs(colDef)
         this.pavoGrid.gridApi.addEventListener('cellValueChanged', this.updatePavo)
         this.pavoGrid.gridApi.addEventListener('firstDataRendered', this.updatePavoColumn)
    }

    private updatePavo = (e) => {
        const detail = e.data
        this.connect.commitEvent('EVENT_UPDATE_PAVO', {DETAILS: detail, VALIDATE: false, IGNORE_WARNINGS:true})
            .then(result => {
                console.log("lumin|pavo|updatePavo =>", result.REPLY)
            })
    }

    private updatePavoColumn = (e) => {
        console.log('lumin|pavo|updatePavoColumn')
        this.pavoGrid.gridApi.sizeColumnsToFit()
    }
}